...
class MapCombo(Customize, QComboBox):
    ...
    def init_editor(self):
        ...
        for value, item in self.mapping().items():
            if isinstance(item, tuple):
                icon, name = item
                self.addItem(QIcon(f":/{self.icon}/{icon}.gif"), name, value) if self.icon else self.addItem(name, value)
            else:
    ...
    def refresh(self):
        ...
        if isinstance(item := self.mapping().get(value, '——'), str):
